...
data Guard = Guard Coord Direction deriving (Show, Eq)

main :: IO ()
main = do
    inputStr <- readFile "input.txt"
    let (obstacles, guard, bounds) = parseInput inputStr
    let visited = visits bounds obstacles guard
    print $ solve visited bounds obstacles guard
...
solve :: Set Coord -> (Int, Int) -> Set Coord -> Guard -> Int
solve visited bounds obstacles guard = length (filter (==True) looped)
    where
        looped = 
            map 
                (\x -> hasLoop bounds (Set.insert x obstacles) guard) 
                (Set.toList visited)
        
        
hasLoop :: (Int, Int) -> Set Coord -> Guard -> Bool
hasLoop bounds obstacles guard = go guard guard
    where
        checkInBounds :: Guard -> Bool
        checkInBounds = guardInArea bounds
        go :: Guard -> Guard -> Bool
        go slow fast
            | fast' == slow'            = True
            | not (checkInBounds fast') = False 
            | otherwise                 = go slow' fast'
            where
                fast' = nextPosition obstacles (nextPosition obstacles fast)
                slow' = nextPosition obstacles slow


visits :: (Int, Int) -> Set Coord -> Guard -> Set Coord
visits bounds obstacles guard = go guard Set.empty
    where
        checkInBounds :: Guard -> Bool
        checkInBounds = guardInArea bounds
        go :: Guard -> Set Coord -> Set Coord
